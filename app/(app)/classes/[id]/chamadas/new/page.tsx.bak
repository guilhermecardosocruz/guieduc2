...
export default function CallNewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setPresentMap(map);
      } catch { setStudents([]); setPresentMap({}); }
    })();
  }, [params]);
  ...
  // handler do modal: adiciona aluno (offline-first) e tenta salvar no servidor
  ...
  return (
    <main className="mx-auto max-w-md px-4 py-6">
      ...
      <div className="mt-3 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
        ...
        {/* toolbar acima da lista: modal + importação por planilha */}
